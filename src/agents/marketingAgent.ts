...
const BRIEF_PROMPT_PATH = path.join(__dirname, '../../prompts/marketingBriefPrompt.txt'); // Renamed
const REPLY_PROMPT_PATH = path.join(__dirname, '../../prompts/marketingReplyPrompt.txt'); // New
    ...
    try {
        ...
        if (parsed && typeof parsed === 'object') {
            ...
            if (typeof parsed.brief === 'string' && parsed.brief.trim()) {
             ...
             // Marketing specific
             if (typeof parsed.perspective === 'string' && parsed.perspective.trim()) {
                 outputLines.push(parsed.perspective.trim());
            }
...
/**
 * Generates the initial brief for the Marketing Agent.
 */
export async function getMarketingAgentBrief(input: AgentBriefInput): Promise<AgentBrief> {
    let template: string;
    try {
        template = await fs.readFile(BRIEF_PROMPT_PATH, 'utf-8');
    } catch (error) {
        console.error(`Error reading prompt template ${BRIEF_PROMPT_PATH}:`, error);
        return "Error: Marketing Agent failed to load brief prompt template.";
    }
...
# Instructions:
Generate a brief, concise (1-3 sentences) opening statement for the Marketing agent based on the context.
Focus on potential messaging, positioning, or growth observations based *only* on the Pulse and Memo provided.
...
`;
    ...
    try {
        ...
        console.log("Marketing Agent brief generated.");
        return brief;
    } catch (error) {
        console.error(`Error during Marketing Agent brief LLM call:`, error);
        return "Critical Error: Exception during Marketing agent brief generation.";
    }
}

/**
 * Generates the follow-up reply for the Marketing Agent based on user response.
 */
export async function getMarketingAgentReply(input: AgentReplyInput): Promise<AgentReply> {
    let template: string;
    try {
        template = await fs.readFile(REPLY_PROMPT_PATH, 'utf-8');
    } catch (error) {
        console.error(`Error reading prompt template ${REPLY_PROMPT_PATH}:`, error);
        return "Error: Marketing Agent failed to load reply prompt template.";
    }
...
Acknowledge the user's input and provide a relevant marketing perspective or clarification.
Do not ask follow-up questions.

# Marketing Agent Reply:
`;

    try {
        const llmResponse = await callLLM(combinedPrompt);
        const reply = parseLLMStringResponse(llmResponse, 'reply');
        console.log("Marketing Agent reply generated.");
        return reply;
    } catch (error) {
        console.error(`Error during Marketing Agent reply LLM call:`, error);
        return "Critical Error: Exception during Marketing agent reply generation.";
    }
}

// --- Deprecated v0.2 Agent --- 
/*
function parseMarketingResponse_v0_2(llmResponse: string | null): AgentStructuredResponse { ... }
export async function runMarketingAgent_v0_2(input: AgentInput_v0_2): Promise<AgentStructuredResponse> { ... }
*/ 