...
const BRIEF_PROMPT_PATH = path.join(__dirname, '../../prompts/productBriefPrompt.txt'); // Renamed
const REPLY_PROMPT_PATH = path.join(__dirname, '../../prompts/productReplyPrompt.txt'); // New
const AGENT_NAME = "Product";
        ...
        if (parsed && typeof parsed === 'object') {
            ...
            // Product specific
             if (typeof parsed.insight === 'string' && parsed.insight.trim()) {
                 outputLines.push(parsed.insight.trim());
            }
...
/**
 * Generates the initial brief for the Product Agent.
 */
export async function getProductAgentBrief(input: AgentBriefInput): Promise<AgentBrief> {
    let template: string;
    try {
        template = await fs.readFile(BRIEF_PROMPT_PATH, 'utf-8');
    } catch (error) {
        console.error(`Error reading prompt template ${BRIEF_PROMPT_PATH}:`, error);
        return "Error: Product Agent failed to load brief prompt template.";
    }
...
# Instructions:
Generate a brief, concise (1-3 sentences) opening statement for the Product agent based on the context.
Focus on potential product implications, UX considerations, or feature clarity based *only* on the Pulse and Memo provided.
...
`;
    ...
    try {
        ...
        console.log("Product Agent brief generated.");
        return brief;
    } catch (error) {
        console.error(`Error during Product Agent brief LLM call:`, error);
        return "Critical Error: Exception during Product agent brief generation.";
    }
}

/**
 * Generates the follow-up reply for the Product Agent based on user response.
 */
export async function getProductAgentReply(input: AgentReplyInput): Promise<AgentReply> {
    let template: string;
    try {
        template = await fs.readFile(REPLY_PROMPT_PATH, 'utf-8');
    } catch (error) {
        console.error(`Error reading prompt template ${REPLY_PROMPT_PATH}:`, error);
        return "Error: Product Agent failed to load reply prompt template.";
    }
...
Acknowledge the user's input and provide a relevant product-focused thought or clarification.
...
`;
    ...
    try {
        ...
        console.log("Product Agent reply generated.");
        return reply;
    } catch (error) {
        console.error(`Error during Product Agent reply LLM call:`, error);
        return "Critical Error: Exception during Product agent reply generation.";
    }
}

// --- Deprecated v0.2 Agent --- 
/*
function parseProductResponse_v0_2(llmResponse: string | null): AgentStructuredResponse { ... }
export async function runProductAgent_v0_2(input: AgentInput_v0_2): Promise<AgentStructuredResponse> { ... }
*/ 