...
import { AgentBriefInput, AgentBrief, AgentReplyInput, AgentReply, AgentStructuredResponse } from '../types.js';
...
const BRIEF_PROMPT_PATH = path.join(__dirname, '../../prompts/strategyBriefPrompt.txt');
const REPLY_PROMPT_PATH = path.join(__dirname, '../../prompts/strategyReplyPrompt.txt');
...
function parseLLMStringResponse(llmResponse: string | null, agentAction: 'brief' | 'reply'): string {
    ...
    try {
        ...
        if (parsed && typeof parsed === 'object') {
            ...
            // Format common structured fields (recommendations, concerns, flags)
...
/**
 * Generates the initial brief for the Strategy Agent.
 */
export async function getStrategyAgentBrief(input: AgentBriefInput): Promise<AgentBrief> {
    let template: string;
    try {
        template = await fs.readFile(BRIEF_PROMPT_PATH, 'utf-8');
    } catch (error) {
        console.error(`Error reading prompt template ${BRIEF_PROMPT_PATH}:`, error);
        return "Error: Strategy Agent failed to load brief prompt template.";
    }

    // Construct prompt for the brief
...
# Instructions:
Generate a brief, concise (1-3 sentences) opening statement for the Strategy agent based on the context.
Focus on potential strategic priorities, trade-offs, or key questions based *only* on the Pulse and Memo provided.
...
`;
    ...
    try {
        ...
        console.log("Strategy Agent brief generated.");
        return brief;
    } catch (error) {
        console.error(`Error during Strategy Agent brief LLM call:`, error);
        return "Critical Error: Exception during Strategy agent brief generation.";
    }
}

/**
 * Generates the follow-up reply for the Strategy Agent based on user response.
 */
export async function getStrategyAgentReply(input: AgentReplyInput): Promise<AgentReply> {
    let template: string;
    try {
        template = await fs.readFile(REPLY_PROMPT_PATH, 'utf-8');
    } catch (error) {
        console.error(`Error reading prompt template ${REPLY_PROMPT_PATH}:`, error);
        return "Error: Strategy Agent failed to load reply prompt template.";
    }

    // Construct prompt for the reply
    const combinedPrompt = `
${template}
...
# Instructions:
Generate a concise (1-3 sentences) follow-up reply based on the user's response, considering the original context and your initial brief. Acknowledge the user's input and provide a relevant strategic thought or clarification. Do not ask follow-up questions.
...
`;
    ...
    try {
        ...
        console.log("Strategy Agent reply generated.");
        return reply;
    } catch (error) {
        console.error(`Error during Strategy Agent reply LLM call:`, error);
        return "Critical Error: Exception during Strategy agent reply generation.";
    }
}

// --- Deprecated v0.2 Agent --- 
// Keep the old function temporarily if other parts of the system still expect it,
// but it should eventually be removed.
/*
function parseStrategyResponse_v0_2(llmResponse: string | null): AgentStructuredResponse { ... original parsing logic ... } 
export async function runStrategyAgent_v0_2(input: AgentInput_v0_2): Promise<AgentStructuredResponse> { ... original run logic ... }
*/ 