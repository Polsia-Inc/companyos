...
const PROMPT_TEMPLATE_PATH = path.join(__dirname, '../../prompts/wellnessPrompt.txt');
 ...
 */
function parseWellnessResponse(llmResponse: string | null): AgentResponse {
    const agentName = "wellness";
...
# Wellness Agent Analysis:
`;

    try {
        const llmResponse = await callLLM(combinedPrompt);
        const structuredResponse = parseWellnessResponse(llmResponse);
        console.log("Wellness Agent finished.");
        return structuredResponse;
    } catch (error) {
        console.error(`Error during Wellness Agent LLM call:`, error);
        return {
            agent: "wellness",
            recommendations: [`Critical Error: Exception during Wellness agent execution.`],
            concerns: [], flags: [], confidence: 0.0,
        };
    }
} 